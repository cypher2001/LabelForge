/**
 * LabelForge Label Designer Application
 * Multi-element label editor with drag, resize, and rotate
 * v116
 */

import { CanvasRenderer, PX_PER_MM } from './canvas.js?v=118';
import { STARTERS, getStarter } from './starters.js?v=1';
...
const state = {
  ...
  grid: {                 // Design grid, spacing in millimetres
    enabled: false,
    snap: true,
    rulers: false,
    sizeMm: 5,
...
function updateUnitDisplay() {
  ...
  // The grid spacing box and the rulers are both expressed in the display unit
  applyGridSettings();
  drawRulers();
}
...
// Storage keys keep the historical phomymo_ prefix: renaming them would
// orphan every saved design and printer definition already in a browser
const THEME_STORAGE_KEY = 'phomymo_theme';
...
function applyTheme(theme) {
  ...
  safeStorageSet(THEME_STORAGE_KEY, dark ? 'dark' : 'light');

  // The icon offers the theme you would switch to, not the one you are in
  $('#theme-icon-dark')?.classList.toggle('hidden', dark);
  $('#theme-icon-light')?.classList.toggle('hidden', !dark);

  const toggle = $('#theme-toggle');
  if (toggle) toggle.title = dark ? 'Switch to light' : 'Switch to dark';

  // The canvas paints its own background, so it has to be told
  state.renderer?.clearCache();
  render();
}
 ...
 *
 * Only snaps when the grid is both shown and set to snap - snapping to an
 * invisible grid makes dragging feel broken rather than helpful.
 *
 * @param {number} value - Position in canvas pixels
 * @returns {number} - Snapped position
 */
function snapToGrid(value) {
  if (!state.grid.enabled || !state.grid.snap) return value;

  const spacing = Number(state.grid.sizeMm) * PX_PER_MM;
...
function saveGridState() {
  safeStorageSet(GRID_STORAGE_KEY, safeJsonStringify(state.grid));
}

/**
 * Restore the grid settings and push them to the renderer
 */
function loadGridState() {
  const stored = safeJsonParse(safeStorageGet(GRID_STORAGE_KEY), null);
  if (stored && typeof stored === 'object') {
    state.grid = { ...state.grid, ...stored };
  }
  applyGridSettings();
}

/**
 * Reflect the grid settings in the renderer and the toolbar
 */
function applyGridSettings() {
  state.renderer?.setGrid({ enabled: state.grid.enabled, sizeMm: state.grid.sizeMm });

  const btn = $('#grid-btn');
  if (btn) {
    const on = state.grid.enabled;
    btn.classList.toggle('lf-toggle-on', on);
    btn.title = on ? 'Hide the grid' : 'Show the grid';
  }

  const snapCheckbox = $('#grid-snap');
  if (snapCheckbox) snapCheckbox.checked = state.grid.snap;

  const sizeInput = $('#grid-size');
  if (sizeInput) sizeInput.value = formatLength(state.grid.sizeMm, getUnit());

  const sizeUnit = $('#grid-size-unit');
  if (sizeUnit) sizeUnit.textContent = getUnit();

  const showCheckbox = $('#grid-show');
  if (showCheckbox) showCheckbox.checked = state.grid.enabled;

  const rulerCheckbox = $('#rulers-show');
  if (rulerCheckbox) rulerCheckbox.checked = state.grid.rulers;
}

const RULER_THICKNESS = 22;

/**
 * Draw the rulers alongside the label, ticked in the current display unit
 *
 * They are positioned from the label's on-screen box rather than the canvas
 * element, so they stay aligned through zoom and the overflow padding.
 */
function drawRulers() {
  const horizontal = $('#ruler-h');
  const vertical = $('#ruler-v');
  if (!horizontal || !vertical) return;

  if (!state.grid.rulers) {
  ...
  // Scale between canvas pixels and screen pixels, after any zoom transform
  const screenPerPx = canvasBox.width / canvas.width;
  const labelLeft = canvasBox.left - areaBox.left + state.renderer.labelOffsetX * screenPerPx;
  const labelTop = canvasBox.top - areaBox.top + state.renderer.labelOffsetY * screenPerPx;
  const labelWidth = state.renderer.labelWidth * state.zoom * screenPerPx;
  const labelHeight = state.renderer.labelHeight * state.zoom * screenPerPx;

  const unit = getUnit();
  const dark = isDarkTheme();
  const ink = dark ? '#98a3b3' : '#6b7280';
  const surface = dark ? '#1c222b' : '#f9fafb';
  const border = dark ? '#2c3441' : '#e5e7eb';

  // A tick every whole unit, thinned out until labels stop colliding
  const pxPerUnit = (isInches(unit) ? MM_PER_INCH : 1) * PX_PER_MM * state.zoom * screenPerPx;
  let stepUnits = 1;
  while (pxPerUnit * stepUnits < 28) stepUnits *= isInches(unit) ? 2 : 5;

  const setup = (el, width, height, left, top) => {
  ...
  // Horizontal
  const hctx = setup(horizontal, labelWidth, RULER_THICKNESS, labelLeft, labelTop - RULER_THICKNESS);
  hctx.textAlign = 'left';
  hctx.textBaseline = 'top';
  for (let u = 0; u * pxPerUnit <= labelWidth + 0.5; u += stepUnits) {
    const x = Math.round(u * pxPerUnit) + 0.5;
    hctx.beginPath();
    hctx.moveTo(x, RULER_THICKNESS - 6);
    hctx.lineTo(x, RULER_THICKNESS);
    hctx.stroke();
    hctx.fillText(String(u), x + 2, 3);
  }

  // Vertical
  const vctx = setup(vertical, RULER_THICKNESS, labelHeight, labelLeft - RULER_THICKNESS, labelTop);
  vctx.textAlign = 'left';
  vctx.textBaseline = 'top';
  for (let u = 0; u * pxPerUnit <= labelHeight + 0.5; u += stepUnits) {
    const y = Math.round(u * pxPerUnit) + 0.5;
    vctx.beginPath();
    vctx.moveTo(RULER_THICKNESS - 6, y);
    vctx.lineTo(RULER_THICKNESS, y);
    vctx.stroke();
    vctx.fillText(String(u), 2, y + 2);
  }
}

// Tallest a thumbnail may be. The frame adds padding around it, and the two
// must agree or a tall label (4x6in) scales past the frame and gets clipped
const GALLERY_THUMB_HEIGHT = 96;
const GALLERY_FRAME_PADDING = 8;

/**
 * Draw a design into a thumbnail canvas, letterboxed to fit
 * @param {HTMLCanvasElement} canvas - Thumbnail canvas
 * @param {Array} elements - Elements to draw
 * @param {Object} labelSize - { width, height } in millimetres
 */
function renderThumbnail(canvas, elements, labelSize) {
  const labelW = labelSize.width * PX_PER_MM;
  const labelH = labelSize.height * PX_PER_MM;

  // Letterbox into a fixed box. Label stock ranges from 40x12mm to 4x6in, so
  // sizing cards to the artwork leaves the grid ragged
  const frame = canvas.parentElement;
  const available = Math.max(1,
    Math.round((frame?.getBoundingClientRect().width || 160) - GALLERY_FRAME_PADDING * 2));
  const scale = Math.min(available / labelW, GALLERY_THUMB_HEIGHT / labelH);
  const width = Math.max(1, Math.round(labelW * scale));
  const height = Math.max(1, Math.round(labelH * scale));
  ...
  try {
    state.renderer.renderAllToContext(ctx, elements, []);
  } catch (error) {
    logError(error, 'renderThumbnail');
  }
  ctx.restore();
}
 ...
 * @param {string} key - data-key value used by the click handler
 * @param {string} title - Card heading
 * @param {string} subtitle - Supporting line
 * @returns {string} - Card markup
 */
function galleryCard(key, title, subtitle) {
  return `
    <button class="gallery-card text-left rounded-lg border border-gray-200 hover:border-gray-300 hover:shadow-md transition-all overflow-hidden bg-white flex flex-col"
            data-key="${escapeHtml(key)}">
      <div class="gallery-frame flex items-center justify-center bg-gray-50" style="height: ${GALLERY_THUMB_HEIGHT + GALLERY_FRAME_PADDING * 2}px; padding: ${GALLERY_FRAME_PADDING}px">
        <canvas class="gallery-thumb lf-paper block bg-white shadow-sm" data-key="${escapeHtml(key)}"></canvas>
      </div>
      <div class="p-2 border-t border-gray-100">
        <div class="text-xs font-semibold text-gray-900 truncate">${escapeHtml(title)}</div>
        <div class="text-[11px] text-gray-500 truncate">${escapeHtml(subtitle)}</div>
      </div>
    </button>
  `;
}
...
function showGalleryDialog() {
  ...
  const saved = $('#gallery-saved');
  const savedEmpty = $('#gallery-saved-empty');

  starters.innerHTML = STARTERS
    .map(s => galleryCard(`starter:${s.id}`, s.name, s.description))
    .join('');

  const designs = listDesigns();
  saved.innerHTML = designs
    .map(d => galleryCard(`design:${d.name}`, d.name,
      `${d.elementCount} element${d.elementCount !== 1 ? 's' : ''}`))
    .join('');
  savedEmpty.classList.toggle('hidden', designs.length > 0);

  $('#gallery-dialog').classList.remove('hidden');

  // Thumbnails need the cards laid out before they can be sized
  requestAnimationFrame(() => {
    Array.from($$('#gallery-dialog .gallery-thumb')).forEach(canvas => {
      const [kind, ...rest] = (canvas.dataset.key || '').split(':');
      const name = rest.join(':');

      if (kind === 'starter') {
        const starter = getStarter(name);
        if (starter) renderThumbnail(canvas, starter.build(), starter.labelSize);
        return;
      }

      const design = loadDesign(name);
      if (design) {
        renderThumbnail(canvas, design.elements || [], design.labelSize || state.labelSize);
      }
    });
  });

  $$('#gallery-dialog .gallery-card').forEach(card => {
    card.addEventListener('click', () => applyGallerySelection(card.dataset.key));
  });
}

/**
 * Resize the label to a starter's size and reflect it in the size dropdown
 * @param {Object} size - { width, height } in millimetres
 */
function applyStarterLabelSize(size) {
  state.labelSize = { width: size.width, height: size.height, round: false };
  state.renderer.setDimensions(size.width, size.height, state.zoom, false);
  state.renderer.clearCache();

  // Match a preset if one has these dimensions, otherwise show it as custom
  const key = Object.keys(LABEL_SIZES).find(k =>
    LABEL_SIZES[k].width === size.width &&
    LABEL_SIZES[k].height === size.height &&
    !LABEL_SIZES[k].round);
  ...
  if (key) {
    ...
    select.value = 'custom';
    $('#custom-size').classList.remove('hidden');
    writeDimension('#custom-width', size.width);
    writeDimension('#custom-height', size.height);
  }
...
function hideGalleryDialog() {
  $('#gallery-dialog').classList.add('hidden');
}

/**
 * Apply whatever the user picked in the gallery
 * @param {string} key - "starter:<id>" or "design:<name>"
 */
function applyGallerySelection(key) {
  if (!key) return;
  ...
  if (state.elements.length > 0 &&
      !confirm('Replace the current label with this template?')) {
    return;
  }

  saveHistory();
  state.elements = starter.build();
  state.selectedIds = [];
  applyStarterLabelSize(starter.labelSize);
  ...
  updatePropertiesPanel();
  render();
  setStatus(`Started from "${starter.name}"`);
}

/**
 * Show the empty state only while the label has nothing on it
 */
function updateEmptyState() {
  const empty = $('#canvas-empty');
  if (!empty) return;
  empty.classList.toggle('hidden', state.elements.length > 0);
}
...
function toggleTheme() {
  ...
  setStatus(isDarkTheme() ? 'Dark theme' : 'Light theme');
}
...
function exportTemplateCSV() {
  ...
  a.href = url;
  a.download = `${state.currentDesignName || 'labelforge-data'}-${Date.now()}.csv`;
  ...
  grid.innerHTML = recordsToPreview.map(idx => {
    ...
    return `
      <div class="preview-thumbnail cursor-pointer hover:ring-2 hover:ring-blue-500 rounded-lg p-2 bg-gray-50"
           data-index="${idx}">
        <canvas class="preview-canvas lf-paper bg-white shadow rounded w-full" data-index="${idx}"></canvas>
  ...
  if (state.isDragging && state.dragStartElements) {
    ...
    switch (state.dragType) {
      case 'move':
        ...
        modifyElement(el.id, { x: snapToGrid(newX), y: snapToGrid(newY) });
...
function handleExport() {
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `labelforge-design-${Date.now()}.json`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
...
function init() {
  ...
  $('#grid-btn')?.addEventListener('click', (e) => {
    e.stopPropagation();
    $('#grid-dropdown')?.classList.toggle('hidden');
  });
  document.addEventListener('click', (e) => {
    if (!e.target.closest('#grid-dropdown') && !e.target.closest('#grid-btn')) {
      $('#grid-dropdown')?.classList.add('hidden');
    }
  });
  $('#grid-show')?.addEventListener('change', (e) => {
    state.grid.enabled = e.target.checked;
    saveGridState();
    applyGridSettings();
    render();
    setStatus(state.grid.enabled ? 'Grid shown' : 'Grid hidden');
  });
  $('#grid-snap')?.addEventListener('change', (e) => {
    state.grid.snap = e.target.checked;
    saveGridState();
    setStatus(state.grid.snap ? 'Snapping to grid' : 'Snapping off');
  });
  $('#rulers-show')?.addEventListener('change', (e) => {
    state.grid.rulers = e.target.checked;
    saveGridState();
    render();
  });
  $('#grid-size')?.addEventListener('change', (e) => {
    // The box is in the display unit; the setting is stored in millimetres
    const mm = toMm(e.target.value, getUnit());
    state.grid.sizeMm = Math.min(Math.max(mm, 0.5), 50);
    saveGridState();
    applyGridSettings();
    render();
  });

  // The inline head script already set data-theme; sync the button to it
  ...
  $('#empty-add-barcode')?.addEventListener('click', () => $('#add-barcode').click());
  $('#empty-load')?.addEventListener('click', () => $('#load-btn').click());
  $('#empty-templates')?.addEventListener('click', showGalleryDialog);
  $('#gallery-btn')?.addEventListener('click', showGalleryDialog);
  $('#gallery-close')?.addEventListener('click', hideGalleryDialog);
  $('#gallery-dialog')?.addEventListener('click', (e) => {
    if (e.target === $('#gallery-dialog')) hideGalleryDialog();
  });
  updateEmptyState();
  ...
  console.log('LabelForge Label Designer initialized');
}
