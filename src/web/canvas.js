/**
 * Canvas rendering and raster conversion for LabelForge label designer
...
export class CanvasRenderer {
  constructor(canvas) {
    ...
    // Design grid: spacing is in millimetres, the unit everything is stored in
    this.grid = { enabled: false, sizeMm: 5 };
  ...
  /**
   * Set the design grid
   * @param {Object} grid - { enabled, sizeMm }
   */
  setGrid(grid) {
  ...
  /**
   * Draw the design grid over the label area
   *
   * Lines land on millimetre multiples in label space, so they stay put
   * relative to the label as the zoom changes rather than drifting.
   */
  drawGrid() {
    if (!this.grid.enabled) return;

    const spacing = Number(this.grid.sizeMm);
    if (!Number.isFinite(spacing) || spacing <= 0) return;

    const step = spacing * PX_PER_MM * this.zoom;
    // Below about four pixels the grid reads as a wash of colour, not a grid
    if (step < 4) return;

    const ctx = this.ctx;
    const width = this.labelWidth * this.zoom;
    const height = this.labelHeight * this.zoom;
    ...
    ctx.strokeStyle = dark ? 'rgba(255, 255, 255, 0.16)' : 'rgba(17, 24, 39, 0.12)';
    ctx.lineWidth = 1;

    // Half-pixel offsets keep hairlines crisp instead of blurred across two rows
  ...
  drawCheckerboard(ctx, x, y, width, height) {
    ...
    // The checkerboard is painted into the canvas rather than styled, so it
    // has to follow the theme itself or it stays a bright slab in dark mode
    const dark = document.documentElement.getAttribute('data-theme') === 'dark';
    const lightColor = dark ? '#232a35' : '#f0f0f0';
    const darkColor = dark ? '#1a2029' : '#d0d0d0';
